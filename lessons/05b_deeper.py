"""
阶段 5b：分类任务·加深对照版（3 隐藏层 2→64→32→16→3，2851 参数）
...
运行：uv run python lessons/05b_deeper.py
"""
...
class ToyClassifier(nn.Module):
    def __init__(self, n_feat=2, n_hidden=16, n_class=3):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_feat, n_hidden * 4),     # 输入 -> 隐藏层 2,64
            nn.ReLU(),
            nn.Linear(n_hidden * 4, n_hidden * 2),#  64,32
            nn.ReLU(),
            nn.Linear(n_hidden * 2, n_hidden),#  32,16 
            nn.ReLU(),
            # 改成 3 个隐藏层（比如 2→64→32→16→3），重跑：
...
fig.savefig(os.path.join(HERE, "classification_result_deep.png"), dpi=120)
print("📊 已保存 classification_result_deep.png")
